...
def render_sheet(
	session: dict, attendees: list, company: dict | None = None, signatures: dict | None = None
) -> bytes:
...
def _detail(sheet, session: dict, company: dict) -> None:
	...
	trainer = session.get("conducted_by_name") or session.get("instructor_name") or "not recorded"
	when = str(session.get("session_date") or "")
	times = " to ".join(part[:5] for part in (session.get("start_time"), session.get("end_time")) if part)
	...
	sheet.box(render.MARGIN + quarter * 2, top, quarter, height, "Time", times or "not recorded")
	sheet.box(
	...
	sheet.box(render.MARGIN, top, quarter, height, "Location", str(session.get("location") or "not recorded"))
	sheet.box(render.MARGIN + quarter, top, quarter, height, "Conducted by", trainer)
	sheet.box(
		render.MARGIN + quarter * 2,
		top,
		quarter,
		height,
		"Provider",
	...
	sheet.box(
		...
		width / 2.0,
		height,
		"Regimes this session counts towards",
		", ".join(session.get("regimes") or []) or "untagged",
	)
	sheet.box(
		render.MARGIN + width / 2.0,
		top,
		width / 2.0,
		height,
		"Training expires",
...
def _row(sheet, index: int, row: dict, capture: bytes | None) -> None:
	...
	sheet.text(
		...
		baseline,
		sheet.clip(
			str(row.get("badge_scan") or "no badge scanned"),
			badge_right - badge_left - 6,
			render.FONT_PLAIN,
			render.SIZE_SMALL,
		),
		render.FONT_PLAIN,
	...
	sheet.text(
		...
		baseline,
		sheet.clip(
			_moment(row.get("scanned_at")),
			scanned_right - scanned_left - 6,
			render.FONT_PLAIN,
			render.SIZE_SMALL,
		),
		render.FONT_PLAIN,
	...
	sheet.text(
		...
		baseline,
		sheet.clip(
			_moment(row.get("signed_at")),
			signed_right - signed_left - 6,
			render.FONT_PLAIN,
			render.SIZE_LABEL,
		),
		render.FONT_PLAIN,
...
def _summary(sheet, session: dict, attendees: list) -> None:
	...
	sheet.bullets(
		[
			f"{len(attendees)} person(s) on this sheet; {counted.get('absent', 0)} recorded as not present.",
