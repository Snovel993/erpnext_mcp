...
def _w4_fields() -> list[str]:
    return [
        ...
        "employer_signer_name", "employer_signer_title", "employer_signed_at",
        "generated_pdf", "generated_pdf_on",
    ]
...
def render_w4_pdf(args: dict) -> ToolResult:
    ...
    summary = (
        f"W-4 {name} rendered onto the IRS form as {file_name} ({len(pdf):,} bytes) and attached"
...
#: useful thing to do.
_W4_REQUIRED_BOXES = (
    ("filing_status", "Step 1(c): filing status"),
)
