...
def list_available_actions(args: dict) -> ToolResult:
	...
	transitions, via, conditions_evaluated = _available_transitions(doc, workflow, current)
	data = {
		"doctype": doctype,
		"name": name,
		"workflow": workflow.name,
...
def advance_workflow(args: dict) -> ToolResult:
	...
	transitions, _via, conditions_evaluated = _available_transitions(doc, workflow, before_state)
...
def _document_and_workflow(doctype: str, name: str):
	...
	return frappe.get_doc(doctype, name), frappe.get_doc("Workflow", workflow_name)
