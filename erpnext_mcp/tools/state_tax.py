...
def get_state_tax_config(args: dict) -> ToolResult:
    ...
    data = {f: (str(getattr(doc, f, None)) if getattr(doc, f, None) is not None else None)
...
def preview_state_withholding(args: dict) -> ToolResult:
    ...
    result = calculate_state_withholding(
        gross_pay, pay_frequency, work_state, filing_status, config, table,
    )
...
def preview_total_payroll_taxes(args: dict) -> ToolResult:
    ...
    result = calculate_all_payroll_taxes(
        ...
        w4_data, ytd_gross, ytd_ss, fica, fed_table, state_config, state_table,
    )
    result["employee"] = employee
    result["tax_year"] = tax_year
    result["gross_pay"] = gross_pay
...
def _config_fields() -> list[str]:
    return [
        "company", "state", "tax_year", "status",
        "or_income_tax_enabled", "or_transit_tax_rate",
        ...
        "wa_pfml_rate", "wa_pfml_employee_share", "wa_pfml_employer_share",
        "wa_pfml_wage_base",
