...
def amount_cents(dollars, width: int = 10) -> str:
	...
	"""
	cents = int(Decimal(str(float(dollars))).scaleb(2).quantize(Decimal("1"), rounding=ROUND_HALF_UP))
	if cents < 0:
...
def normalize_routing(routing) -> str:
	...
	if len(digits) != 9:
		raise NachaError(f"routing number {routing!r} has {len(digits)} digits; an ABA routing number has 9.")
...
def build_file(
	...
	origin = str(originator.get("immediate_origin") or "").strip() or originator["company_identification"]
