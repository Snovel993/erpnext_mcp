...
class NachaFileTool(ACHTestCase):
	...
	def test_an_unresolvable_allocation_refuses_the_whole_file(self):
		...
		self._add_account("HR-EMP-00001", allocation_type="Fixed Amount", allocation_amount=50)
		entry = self._seed_payroll_entry(
			slips=[
	...
	def test_a_split_deposit_makes_two_entries(self):
		self._add_account("HR-EMP-00001", allocation_type="Fixed Amount", allocation_amount=200, priority=1)
		self._add_account(
			"HR-EMP-00001",
			routing_number=ROUTING_B,
			account_number="998877",
...
class MobileBankAccounts(MobileAPITestCase):
	...
	def test_the_colleagues_account_is_untouched_after_the_refusal(self):
		...
		"""
		victim = self._add(OUTSIDER, routing=ROUTING_B, account="33334444")
		before = [dict(r) for r in STORE.rows("Employee Bank Account") if r.get("name") == victim["name"]]
		self.assertEqual(len(before), 1)
		self.be(WORKER)
		with self.assertRaises(Exception):
			mobile_api.update_my_bank_account(name=victim["name"], account_number="99998888")
		after = [dict(r) for r in STORE.rows("Employee Bank Account") if r.get("name") == victim["name"]]
		if after:
