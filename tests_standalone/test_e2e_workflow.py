...
from erpnext_mcp import audit_packets, compliance_fields, compliance_rules, roles
...
SWITCHES = {
	...
	for name in (
		...
		"list_detector_tests",
		# v0.22.1. The whole-sweep parity check below needs to be able to WRITE
		# the records the five migrated rules read, so the walk covers them with
		# the same tools an operation uses rather than by reaching into the store.
...
class TheWholeSweepSaysWhatItSaidBeforeTheRulesBecameData(EndToEndWorkflow):
	"""v0.22.1. One operation, one night, the WHOLE calendar — before and after.

	    `test_ccf_primitives.TheMigrationOfEachRuleChangesNothing` checks each of the
	    five migrated rules on a fixture built to make that one rule speak. This
	    checks something the per-rule tests cannot: that on a REAL operation, with
	    every rule running together against the same records, the calendar an operator
	    opens in the morning is the same calendar.

	    It matters because the interesting failures of a migration like this are not
	    inside one rule. They are two rules that used to read a doctype in a
	    particular order, a company scoping that used to come from a scanner's own
	    `_company_filter` and now comes from the engine, a category that used to be
	    written by one rule and defaulted by another. None of that is visible from a
	    fixture with one cabin in it.

	The comparison is against the SHIPPED SCANNERS, which are still here as the
	    pre-migration fallback — so the golden snapshot is generated by the code
	    v0.22.0 actually ran rather than by a file somebody pasted a run into and
	    which nothing then keeps honest.
	"""

	def days_out(self, days: int) -> str:
		return str(frappe.utils.add_days(frappe.utils.getdate(frappe.utils.today()), days))

	def a_whole_season(self) -> None:
		"""Records that make all five migrated rules speak at once, through the tools."""
		self.tool_data(
			"create_certification",
			{
				...
				"expiration_date": self.days_out(45),
		...
		self.tool_data(
			"create_certification",
			{
				...
				"expiration_date": self.days_out(-12),
			},
		)
		block = self.tool_data(
			"create_field", {"parcel": PARCEL, "field_name": "E2E Block 1", "acreage": 12.5}
		)["name"]
		row = frappe.get_doc("Field", block)
		frappe.db.set_value(
			"Field",
			block,
			{
				"last_spray_date": self.days_out(-11),
				"water_test_last_date": self.days_out(-118),
			},
		...
		frappe.db.set_value(
			...
			self.unit,
			{
				"smoke_detector_last_test": self.days_out(-400),
				"co_detector_last_test": self.days_out(-30),
			},
			update_modified=False,
		)
		self.tool_data(
			...
			{
				"unit": self.unit,
				"inspection_date": self.days_out(-20),
				"findings": "water stain, north wall, spreading",
			},
		)
		# A SECOND block for the water sample, and the reason is a real behaviour
		# of this app rather than a fixture convenience: `create_water_test`
		# refreshes the block's `water_test_last_date`, so sampling Block 1 would
		# silence `water_test_stale` on it — correctly. The two rules are about
		# ignorance and knowledge respectively, and a fixture that made one hide
		# the other would test neither.
		self.tool_data("create_field", {"parcel": PARCEL, "field_name": "E2E Block 2", "acreage": 8.0})
		zone = self.tool_data(
			"create_irrigation_zone",
			{
				"field": "E2E Block 2",
				"zone_name": "E2E-Zone-1",
				"zone_number": 1,
				"water_source": "creek",
			},
		)["name"]
		self.tool_data(
			...
			{
				"source": zone,
				"test_date": self.days_out(-6),
				"coliform_result": "Present",
			},
		)

	def calendar_rows(self) -> dict:
		"""Every alert on the calendar, keyed on its docname, as the sweep wrote it."""
		self.tool_data("refresh_compliance_alerts", {"company": COMPANY})
		out = {}
		for row in STORE.rows("Compliance Alert"):
			out[str(row["name"])] = {
	...
	def test_the_whole_calendar_is_identical_before_and_after_the_five_migrations(self):
		self.be_the_operator()
		self.a_whole_season()
		compliance_rules.seed_compliance_rules()
		...
		# ── the golden snapshot: the whole sweep, with the five rules running as
		#    the scanners v0.22.0 shipped. Not a pasted file — the code itself.
		for rule_id in migrated_ids:
			STORE.get_raw(compliance_rules.DOCTYPE, compliance_rules.resolve(rule_id))["builtin_scanner"] = (
				rule_id
			)
		golden = self.calendar_rows()

		fired = {row["alert_type"] for row in golden.values()}
		for rule_id in migrated_ids:
			self.assertIn(
				rule_id,
				fired,
				f"{rule_id} did not fire in the whole-season fixture, so this walk proves nothing "
				f"about its migration",
			)

		# ── and again, with all thirteen as this release ships them.
		STORE.tables["Compliance Alert"] = {}
		for rule_id in migrated_ids:
			STORE.get_raw(compliance_rules.DOCTYPE, compliance_rules.resolve(rule_id))["builtin_scanner"] = ""
		after = self.calendar_rows()

		self.assertEqual(
			sorted(golden),
			sorted(after),
			"the whole calendar's set of alert docnames changed across v0.22.1. A docname carries "
			"the rule and the record and nothing that moves daily — if it changed, every snooze and "
			"every human dismissal on this operation has been orphaned.",
		)
		for name in sorted(golden):
			with self.subTest(alert=name):
				self.assertEqual(
					after[name],
					golden[name],
					f"{name} says something different after v0.22.1. Five rule definitions moved out "
					f"of Python and into records; what the calendar SAYS was not supposed to move "
					f"with them.",
				)

	def test_the_calendar_an_operator_reads_groups_the_same_way(self):
		"""The rows are identical above; this is the READ over them, because the
		category a migrated rule writes is what decides which group somebody's
		afternoon lands in."""
		self.be_the_operator()
		self.a_whole_season()
		compliance_rules.seed_compliance_rules()
		self.tool_data("refresh_compliance_alerts", {"company": COMPANY})
		calendar = self.tool_data("get_compliance_calendar", {"company": COMPANY})
		by_category = {
			key: sorted(alert["alert_type"] for alert in group["alerts"])
			for key, group in calendar["by_category"].items()
		}
		self.assertIn("certification_expiring", by_category.get("Certifications", []))
		self.assertIn(
			"certification_expiring",
			by_category.get("Workforce", []),
			"an applicator licence is a Workforce item — category_heuristics_json is what keeps it "
			"one now that the rule is data",
		)
		self.assertIn("housing_corrective_action_open", by_category.get("Housing", []))
		self.assertIn("water_test_contamination", by_category.get("Water and Sanitation", []))
